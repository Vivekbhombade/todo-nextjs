...
function CreateTodoForm() {
  ...
  const [editId, setEditId] = useState(null); // ✅ Track ID of item being edited
  ...
  const handleSubmit = async (e) => {
    ...
    if (editId) {
      ...
      const { error } = await supabase
        .from('todolist')
        .insert([{ title: title.trim(), rating: Number(rating) }]);
  ...
  return (
    <div className="max-w-md mx-auto mt-10">
        <h1 className='text-center text-2xl mb-2'>This is Todo App</h1>
      ...
      <div className="mt-10">
        ...
        {todos.length === 0 ? (
          ...
          <ul className="space-y-2">
            {todos.map((todo) => (
              <li key={todo.id} className="border p-3 rounded bg-gray-50">
